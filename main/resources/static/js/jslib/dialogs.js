...
ctrlrs.controller('waitDialogCtrl',['$scope','$modalInstance','$translate','$timeout','data',function($scope,$modalInstance,$translate,$timeout,data){
	...
	$scope.loading = (angular.isDefined(data.loading)) ? data.loading : true;
	
	$scope.isLoadSuccess = (angular.isDefined(data.isLoadSuccess)) ? data.isLoadSuccess : false;
	
	$scope.isLoadFail = (angular.isDefined(data.isLoadFail)) ? data.isLoadFail : false;
	
	$scope.loadSuccessMsg = (angular.isDefined(data.loadSuccessMsg)) ? data.loadSuccessMsg : "Done";
	
	$scope.loadFailMsg = (angular.isDefined(data.loadFailMsg)) ? data.progress : "Failed";
	...
	$scope.$on('dialogs.wait.completeSucess',function(){
		$scope.isLoadSuccess = true;
		$scope.loading = false;
		$timeout(function(){ $modalInstance.close(); $scope.$destroy(); } , 3000);
	}); // end on(dialogs.wait.complete)
	
	
	$scope.$on('dialogs.wait.completeError',function(){
		$scope.isLoadFail = true;
		$scope.loading = false;
		$timeout(function(){ $modalInstance.close(); $scope.$destroy(); }, 3000);
	}); // end on(dialogs.wait.complete)
	...
	.provider('dialogs',[function(){
		...
		this.$get = ['$modal',function ($modal){
			...
			return {
				...
				spin : function(header,msg,progress,opts){
					opts = _setOpts(opts);

					return $modal.open({
						templateUrl : '/dialogs/loading.html',
						controller : 'waitDialogCtrl',
						backdrop: opts.bd,
						keyboard: opts.kb,
						windowClass: opts.wc,
						...
						resolve : {
							data : function(){
								return {
									...
									msg : angular.copy(msg),
									progress : angular.copy(progress),
									isLoadSuccess : angular.copy(opts.isLoadSuccess),
									isLoadFail : angular.copy(opts.isLoadFail),
									loading : angular.copy(opts.loading),
									loadSuccessMsg : angular.copy(opts.loadSuccessMsg),
									loadFailMsg : angular.copy(opts.loadFailMsg)
...
angular.module('dialogs.main',['dialogs.services','ngSanitize']) // requires angular-sanitize.min.js (ngSanitize) //code.angularjs.org/1.2.1/angular-sanitize.min.js
	...
	.run(['$templateCache','$interpolate',function($templateCache,$interpolate){
		...
		$templateCache.put('/dialogs/loading.html','<div class="modal-header dialog-header-wait"><h4 class="modal-title"><span class="fa fa-clock-o"></span> '+startSym+'header'+endSym+'</h4></div><div class="modal-body"><div id="loadingDiv" ng-show="loading"><p ng-bind-html="msg"></p><span class="fa fa-spinner fa-pulse fa-3x fa-fw margin-bottom"></span></div><div id="loadSuccess" ng-show="isLoadSuccess"><p ng-bind-html="loadSuccessMsg"></p><span class="fa fa-check-circle fa-3x fa-fw margin-bottom"></span></div><div id="loadFail" ng-show="isLoadFail"><p ng-bind-html="loadFailMsg"></p><span class="fa fa-times-circle fa-3x fa-fw margin-bottom"></span></div><span class="sr-only">'+startSym+'progress'+endSym+''+startSym+'"DIALOGS_PERCENT_COMPLETE" | translate'+endSym+'</span></div></div>');
