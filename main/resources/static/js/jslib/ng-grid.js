...
var ngAggregate = function (aggEntity, rowFactory, rowHeight, groupInitState,merge) {
    this.rowIndex = 0;
...
ngAggregate.prototype.mergeRow = function (event) {
...
ngAggregate.prototype.copy = function () {
    var ret = new ngAggregate(this.entity, this.rowFactory, this.rowHeight, this.groupInitState,this.merge);
    ret.orig = this;
    return ret;
};
var ngColumn = function (config, $scope, grid, domUtilityService, $templateCache, $utils) {
    ...
    self.pinnable = false;
	self.groupFixed = $utils.isNullOrUndefined(colDef.groupFixed) || colDef.groupFixed;;
    ...
    self.setVars = function (fromCol) {
		...
		self.groupFixed = fromCol.groupFixed;
...
var ngRowFactory = function (grid, $scope, domUtilityService, $templateCache, $utils) {
    ...
    self.buildAggregateRow = function(aggEntity, rowIndex) {
        ...
        if (!agg) {
            // build the row
            agg = new ngAggregate(aggEntity, self, self.rowConfig.rowHeight, grid.config.groupsCollapsedByDefault, grid.config.merge);
    ...
    self.toggleSelectAll = function (checkAll, bypass, selectFiltered) {
        ...
        if (bypass || grid.config.beforeSelectionChange(rows, checkAll)) {
            ...
            for (var i = 0; i < rows.length; i++) {
                ...
                if (checkAll) {self.selectedItems.push(rows[i].entity);
                }
            }
            if (!bypass) {
...
angular.module('ngGrid').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('aggregateTemplate.html',
    ...
    "\n" +
	"    <span class=\"ngAggregateText\" style='left:65px'><button type='button' title='Merge' ng-click='row.mergeRow($event)' class='btn btn-info btn-xs'><i class='fa fa-exchange'></i></button></span>\r" +
    "\n" +
  ...
  $templateCache.put('gridTemplate.html',
    ...
    "\n" +
    "                        <span ng-show=\"group.groupFixed\"  ng-click=\"removeGroup($index)\" class=\"ngRemoveGroup\">x</span>\r" +
    "\n" +
    "                    </span>\r" +
    "\n" +
